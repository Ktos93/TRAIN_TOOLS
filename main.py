...
bezier control points (pad/truncate when the point count changes) and
refreshes the Point Info panel whenever the selected point changes.
"""
import bpy
...
def depsgraph_update_post(scene, depsgraph):
    ...
    if obj is None or obj.type != 'CURVE':
        ...
        track = helpers.get_selected_track(scene)
    ...
    # Keep the per-point record list sized to the control points.
    total = len(spline.bezier_points)
    if len(curve_data.train_points) != total:
        storage.ensure_point_records(curve_data, total)
